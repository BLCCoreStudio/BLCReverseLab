...
from .version_intel import build_version_intelligence
...
input,button,select{font:inherit}input,select{width:100%;background:#070b12;border:1px solid #2b3b50;color:var(--text);border-radius:10px;padding:10px 11px;outline:none}input:focus,select:focus{border-color:#527aa8;box-shadow:0 0 0 3px #31577c33}button{cursor:pointer;border:1px solid #31445c;background:#111b28;color:var(--text);padding:9px 12px;border-radius:10px}button:hover{border-color:#5a7ea7}.analysis{width:100%;text-align:left;margin:5px 0}.analysis.active{background:#1a2b40;border-color:#6590be}.analysis strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.analysis small{display:block;margin-top:3px;color:var(--muted)}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(135px,1fr));gap:9px;margin:12px 0}.metric{padding:12px;border:1px solid #233246;background:#080e16;border-radius:12px}.metric span{display:block;font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.08em}.metric strong{display:block;margin-top:5px;font-size:19px}.section{margin-top:16px;border-top:1px solid var(--line);padding-top:14px}.section h3{font-size:15px;margin:0 0 9px}.toolbar{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px}.comparebar{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;margin:10px 0}.muted{color:var(--muted)}code{color:#c4deff}.answer,.focus{margin-top:10px;padding:13px;border:1px solid #2c425c;background:#09131f;border-radius:12px;line-height:1.5}.support{display:grid;gap:6px;margin-top:8px}.support-item{padding:8px 10px;background:#080e16;border:1px solid #202d3f;border-radius:9px;font-size:12px}.support-item b{color:var(--blue)}
.tabs{display:flex;gap:7px;flex-wrap:wrap;margin:12px 0}.tabs button.active{background:#1b3049;border-color:#6795c4}.view{display:none}.view.active{display:block}.graph-wrap{border:1px solid #223147;background:#060a10;border-radius:13px;overflow:auto;min-height:480px}.graph-wrap svg{min-width:1100px;display:block}.legend{display:flex;flex-wrap:wrap;gap:8px;margin:8px 0}.legend span,.surface{font-size:11px;border:1px solid #2c3b4e;border-radius:999px;padding:4px 7px}.table{width:100%;border-collapse:collapse;font-size:12px}.table th,.table td{text-align:left;padding:8px;border-bottom:1px solid #1c2939;vertical-align:top}.table th{color:var(--muted)}pre{background:#070b11;border:1px solid #1e2b3c;border-radius:10px;padding:10px;white-space:pre-wrap;word-break:break-word;max-height:420px;overflow:auto}.timeline{display:flex;gap:7px;overflow:auto;padding:8px 2px 12px}.tick{min-width:145px;text-align:left}.tick.current{border-color:#75a5d6;background:#17283b}.tick small{display:block;color:var(--muted);margin-top:3px}.changes{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:9px}.change{border:1px solid #223147;background:#080e16;border-radius:11px;padding:11px}.change b{display:block;margin-bottom:5px}.added{color:var(--green)}.removed{color:var(--danger)}.impact-high{color:var(--danger)}.impact-medium{color:var(--gold)}.impact-low{color:var(--blue)}
@media(max-width:900px){.shell{grid-template-columns:1fr}.side{position:static;height:auto}.graph-wrap{min-height:360px}.comparebar{grid-template-columns:1fr}}
</style></head>
...
function shortName(a){return (a.target||'').split('/').pop()||a.sha256}
function list(){const q=document.getElementById('filter').value.toLowerCase(),box=document.getElementById('analyses');box.innerHTML='';for(const a of S.workspace.analyses){const hay=`${a.target} ${a.sha256} ${a.file_type||''}`.toLowerCase();if(!hay.includes(q))continue;const b=document.createElement('button');b.className='analysis'+(S.current===a.sha256?' active':'');b.disabled=!a.available;b.innerHTML=`<strong>${esc(shortName(a))}</strong><small>${esc(a.file_type||'unknown')} · ${esc(a.sha256.slice(0,10))}</small>`;b.onclick=()=>select(a.sha256);box.appendChild(b)}}
function timeline(){return `<div class="timeline">${S.workspace.analyses.filter(a=>a.available).map(a=>`<button class="tick ${a.sha256===S.current?'current':''}" data-sha="${esc(a.sha256)}"><b>${esc(shortName(a))}</b><small>${esc(a.sha256.slice(0,10))}</small></button>`).join('')}</div>`}
function baselineOptions(){const available=S.workspace.analyses.filter(a=>a.available&&a.sha256!==S.current);return available.map(a=>`<option value="${esc(a.sha256)}">${esc(shortName(a))} · ${esc(a.sha256.slice(0,10))}</option>`).join('')}
async function select(sha){S.current=sha;list();S.analysis=await api('/api/analysis/'+encodeURIComponent(sha));S.graph=await api('/api/graph?sha='+encodeURIComponent(sha));document.getElementById('empty').hidden=true;const w=document.getElementById('workspace');w.hidden=false;const f=S.analysis.facts||{},g=f.ghidra||{},x=f.jni_crossrefs||{},r=f.recovery||{},p=f.protection||{};w.innerHTML=`<h2>${esc((S.analysis.target||'').split('/').pop()||sha)}</h2><p class="muted"><code>${esc(sha)}</code></p>${timeline()}<div class="grid">${metric('Artifact',S.analysis.file_type||'unknown')}${metric('Evidence',(S.analysis.evidence||[]).length)}${metric('Graph nodes',S.graph.stats?.node_count||0)}${metric('Native funcs',g.function_count||0)}${metric('JNI links',x.matched_declaration_count||0)}${metric('Protection',p.level||'none')}${metric('Obfuscation',r.obfuscation_score||0)}</div><div class="section"><h3>Evidence Analyst</h3><div class="toolbar"><input id="askq" placeholder="Where is inventory handled? Which JNI bridge reaches native code?"><button id="askbtn">Ask</button></div><div id="answer" class="muted">Answers are grounded only in this saved analysis.</div></div><div class="section"><div class="tabs"><button class="active" data-tab="graph">Cross-layer graph</button><button data-tab="version">Version intelligence</button><button data-tab="search">Universal search</button><button data-tab="raw">Raw facts</button></div><div id="tab-graph" class="view active"><div class="legend"><span>managed</span><span>JNI</span><span>native</span><span>network</span><span>recovery</span><span>evidence</span></div><div id="graph" class="graph-wrap"></div></div><div id="tab-version" class="view"><div class="comparebar"><select id="baseline"><option value="">Select baseline build</option>${baselineOptions()}</select><button id="comparebtn">Compare → current</button></div><div id="versionout" class="muted">Choose an older saved analysis to compare against the current build.</div></div><div id="tab-search" class="view"><div class="toolbar"><input id="searchq" placeholder="Search classes, functions, endpoints, evidence"><button id="searchbtn">Search</button></div><div id="searchout"></div></div><div id="tab-raw" class="view"><pre>${esc(JSON.stringify(f,null,2))}</pre></div></div>`;
for(const b of w.querySelectorAll('.tick'))b.onclick=()=>select(b.dataset.sha);document.getElementById('askbtn').onclick=ask;document.getElementById('askq').addEventListener('keydown',e=>{if(e.key==='Enter')ask()});document.getElementById('searchbtn').onclick=search;document.getElementById('searchq').addEventListener('keydown',e=>{if(e.key==='Enter')search()});document.getElementById('comparebtn').onclick=compareVersion;for(const b of w.querySelectorAll('[data-tab]'))b.onclick=()=>tab(b.dataset.tab);chooseBaseline();drawGraph(S.graph)}
function chooseBaseline(){const sel=document.getElementById('baseline');if(!sel||sel.options.length<2)return;const currentIndex=S.workspace.analyses.findIndex(a=>a.sha256===S.current);for(let i=currentIndex-1;i>=0;i--){const a=S.workspace.analyses[i];if(a.available){sel.value=a.sha256;return}}sel.selectedIndex=1}
...
function countChanges(section){return (section?.added?.length||0)+(section?.removed?.length||0)}
function changeBlock(title,data){const add=(data?.added||[]).slice(0,25),rem=(data?.removed||[]).slice(0,25);if(!add.length&&!rem.length)return `<div class="change"><b>${esc(title)}</b><span class="muted">No indexed changes.</span></div>`;return `<div class="change"><b>${esc(title)}</b>${add.map(x=>`<div class="added">+ ${esc(x)}</div>`).join('')}${rem.map(x=>`<div class="removed">− ${esc(x)}</div>`).join('')}</div>`}
async function compareVersion(){const before=document.getElementById('baseline').value;if(!before)return;const out=document.getElementById('versionout');out.textContent='Comparing saved evidence…';try{const d=await api('/api/version?before='+encodeURIComponent(before)+'&after='+encodeURIComponent(S.current));const m=d.managed||{},n=d.native||{},j=d.jni||{},p=d.protection||{},r=d.recovery||{},s=d.android_structure||{};const surfaces=(d.changed_surfaces||[]).map(x=>`<span class="surface">${esc(x)}</span>`).join(' ')||'<span class="muted">none</span>';const focus=(d.focus||[]).map(x=>`<div class="focus"><b>${esc(x.surface)}</b><div class="muted">${esc(x.reason)}</div></div>`).join('')||'<p class="muted">No focused re-analysis area was derived from the available evidence.</p>';out.innerHTML=`<div class="grid">${metric('Impact',d.impact)}${metric('Changed surfaces',(d.changed_surfaces||[]).length)}${metric('Analysis reuse',Math.round((d.analysis_reuse_ratio||0)*100)+'%')}${metric('Native reuse',Math.round((n.reuse_ratio||0)*100)+'%')}${metric('New methods',m.methods?.added?.length||0)}${metric('New endpoints',m.endpoints?.added?.length||0)}${metric('JNI matched',`${j.matched_before||0} → ${j.matched_after||0}`)}${metric('Obfuscation',`${r.obfuscation_before||0} → ${r.obfuscation_after||0}`)}</div><p><b>Changed surfaces:</b> ${surfaces}</p><div class="changes">${changeBlock('Managed classes',m.classes)}${changeBlock('Managed methods',m.methods)}${changeBlock('Endpoints',m.endpoints)}${changeBlock('JNI bridges',j.bridges)}${changeBlock('Protection signals',p.signals)}${changeBlock('Native libraries',{added:n.added_libraries,removed:n.removed_libraries})}${changeBlock('DEX files',{added:s.added_dex,removed:s.removed_dex})}</div><div class="section"><h3>Re-analysis focus</h3>${focus}</div><p class="muted">${esc(d.caveat||'')}</p>`}catch(e){out.innerHTML=`<div class="answer impact-high">${esc(e.message)}</div>`}}
        ...
        try:
            ...
            if parsed.path == "/api/version":
                ...
                if not before or not after:
                    ...
                    return
                self._json(build_version_intelligence(self.store.load_analysis(before), self.store.load_analysis(after)))
                return
