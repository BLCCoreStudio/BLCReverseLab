...
def main() -> None:
    ...
    search = sub.add_parser("search", help="Universal search across a saved analysis"); search.add_argument("analysis"); search.add_argument("query"); search.add_argument("--limit", type=int, default=100)
    ...
    if args.command == "analyze":
        pipeline = build_pipeline(enable_jadx=args.jadx or args.recover, enable_recovery=args.recover, enable_ghidra=args.ghidra, deep=args.deep, mappings_path=args.mapping, workdir=args.workdir, ghidra_timeout=args.ghidra_timeout, ghidra_max_native=args.ghidra_max_native)
        ctx = pipeline.analyze(args.target); out = Pipeline.save(ctx, args.output); payload = ctx.to_dict(); print(json.dumps(payload, indent=2, sort_keys=True)); print(f"\nSaved: {out}")
    ...
    if args.command == "bundle": print(f"Saved: {create_bundle(_load(args.analysis), args.output, version_diff=_load(args.version_diff) if args.version_diff else None)}"); return
    if args.command == "search": print(json.dumps(search_report(_load(args.analysis), args.query, limit=max(1,args.limit)), indent=2, sort_keys=True)); return
    if args.command == "doctor": print(json.dumps(doctor(), indent=2, sort_keys=True)); return
    if args.command == "enrich":
        enriched=enrich_with_runtime_observations(_load(args.analysis),_load(args.runtime)); output=Path(args.output); output.parent.mkdir(parents=True,exist_ok=True); output.write_text(json.dumps(enriched,indent=2,sort_keys=True)+"\n",encoding="utf-8"); print(f"Saved: {output}"); return
