...
class JadxResult:
    ...
    stderr_tail: str = ""
    deobfuscation_requested: bool = False
...
class JadxAdapter:
    ...
    def build_command(self, binary: str, target: Path, output_dir: Path) -> list[str]:
        command = [binary, "--output-dir", str(output_dir)]
        if self.deobfuscate:
            command.extend(
                [
                    "--deobf",
        ...
        if self.mappings_path is not None:
            command.extend(["--mappings-path", str(self.mappings_path)])
    ...
    def analyze(self, target: Path, output_dir: Path) -> JadxResult:
        ...
        if not binary:
            return JadxResult(
                available=False,
                status="unavailable",
                deobfuscation_requested=self.deobfuscate,
                mappings_path=str(self.mappings_path) if self.mappings_path else None,
            )

        if self.mappings_path is not None and not self.mappings_path.exists():
            return JadxResult(
                available=True,
